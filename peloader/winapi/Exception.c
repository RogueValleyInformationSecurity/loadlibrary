...
extern struct pe_image image __attribute__((weak));
...
EXCEPTION_DISPOSITION WINAPI ExceptionHandler(struct _EXCEPTION_RECORD *ExceptionRecord,
    ...
    PVOID addr = ExceptionRecord ? ExceptionRecord->ExceptionAddress : NULL;

    LogMessage("Toplevel Exception Handler Caught Exception %#x at %p", code, addr);
    abort();
}
...
STATIC bool IsInImageRange(const void *ptr, size_t len) {
    const uint8_t *base = (const uint8_t *)image.image;
    const uint8_t *end = base + image.size;
...
STATIC const char *GetCxxExceptionTypeName(const EXCEPTION_RECORD *Record) {
    struct ms_throw_info {
        uint32_t attributes;
        int32_t pmfnUnwind;
    ...
    struct ms_catchable_type_array {
        int32_t count;
        int32_t types[1];
    };

    struct ms_catchable_type {
        uint32_t properties;
        int32_t pType;
        int32_t thisDisp;
    ...
    struct ms_type_descriptor {
        void *vftable;
        void *spare;
        char name[1];
    };

    if (!Record || Record->NumberParameters < 3) {
        return NULL;
    }
    if (Record->ExceptionInformation[0] != 0x19930520) {
        return NULL;
    }

    const void *throw_info_ptr = (const void *)Record->ExceptionInformation[2];
    if (!IsInImageRange(throw_info_ptr, sizeof(struct ms_throw_info))) {
        return NULL;
    }

    const struct ms_throw_info *throw_info = (const struct ms_throw_info *)throw_info_ptr;
    uintptr_t base = (uintptr_t)image.image;
    uintptr_t cta_addr = (uintptr_t)throw_info->pCatchableTypeArray;
    if (!IsInImageRange((void *)cta_addr, sizeof(struct ms_catchable_type_array))) {
        cta_addr = base + (uintptr_t)throw_info->pCatchableTypeArray;
        if (!IsInImageRange((void *)cta_addr, sizeof(struct ms_catchable_type_array))) {
            return NULL;
        }
    }

    const struct ms_catchable_type_array *cta = (const struct ms_catchable_type_array *)cta_addr;
    if (cta->count <= 0) {
        return NULL;
    }

    uintptr_t ct_addr = (uintptr_t)cta->types[0];
    if (!IsInImageRange((void *)ct_addr, sizeof(struct ms_catchable_type))) {
        ct_addr = base + (uintptr_t)cta->types[0];
        if (!IsInImageRange((void *)ct_addr, sizeof(struct ms_catchable_type))) {
            return NULL;
        }
    }

    const struct ms_catchable_type *ct = (const struct ms_catchable_type *)ct_addr;
    uintptr_t td_addr = (uintptr_t)ct->pType;
    if (!IsInImageRange((void *)td_addr, sizeof(struct ms_type_descriptor))) {
        td_addr = base + (uintptr_t)ct->pType;
        if (!IsInImageRange((void *)td_addr, sizeof(struct ms_type_descriptor))) {
            return NULL;
        }
    }

    const struct ms_type_descriptor *td = (const struct ms_type_descriptor *)td_addr;
    if (!IsInImageRange(td->name, 1)) {
        return NULL;
    }
...
STATIC WINAPI VOID RtlCaptureContext(PCONTEXT ContextRecord)
{
    ucontext_t Context;
    ...
    memset(ContextRecord, 0, sizeof(*ContextRecord));
    nix_2_ms_context_swap(&Context, ContextRecord);
}
...
PVOID RtlPcToFileHeader(PVOID PcValue, PVOID *BaseOfImage) {
    ...
    if ((ULONG_PTR) &image == 0 || image.image == NULL || image.size == 0) {
        *BaseOfImage = NULL;
        return NULL;
    }
...
PRUNTIME_FUNCTION RtlLookupFunctionTable(DWORD64 ControlPc,
    ...
    PVOID Table = NULL;
    if ((ULONG_PTR) &image == 0 || image.nt_hdr == NULL) {
        return NULL;
    }
...
RtlDispatchException(PEXCEPTION_RECORD ExceptionRecord, CONTEXT *pContext) {
    ...
    while (TRUE) {
        ...
        if (ExceptionRoutine != NULL) {
            ...
            static int disp_logged;
            if (disp_logged < 4) {
                LogMessage("RtlDispatchException: handler=%p disposition=%d",
                           ExceptionRoutine,
                           Disposition);
                disp_logged++;
            }
            ...
            switch (Disposition) {
                case ExceptionContinueExecution:
                    RtlRestoreContext(pContext, ExceptionRecord);
                    return TRUE;
...
RaiseException(DWORD dwExceptionCode, DWORD dwExceptionFlags, DWORD nNumberOfArguments, PVOID Arguments) {
    ...
    bool cxx_exception = (dwExceptionCode == 0xE06D7363);
    bool swallow_cxx = true;
    const char *type_name = NULL;
    const char *swallow_env = getenv("MPCLIENT_SWALLOW_CXX");

    // Setup Record
    memcpy(&ExceptionRecord.ExceptionInformation, Arguments, nNumberOfArguments * sizeof(long));

    if (swallow_env && *swallow_env != '\0' && strcmp(swallow_env, "0") == 0) {
        swallow_cxx = false;
    }

    const char *lua_match = NULL;
    const char *chres_match = NULL;
    const char *rt_match = NULL;
    bool swallow_type = false;
    if (cxx_exception) {
        type_name = GetCxxExceptionTypeName(&ExceptionRecord);
        if (type_name) {
            LogMessage("RaiseException: C++ exception type %s", type_name);
            lua_match = strstr(type_name, "lua_longjmp");
            chres_match = strstr(type_name, "CHResultExceptionImpl");
            rt_match = strstr(type_name, "runtime_error");
            LogMessage("RaiseException: match lua_longjmp=%p chresult=%p runtime_error=%p",
                       (const void *)lua_match,
                       (const void *)chres_match,
                       (const void *)rt_match);
            swallow_type = chres_match || rt_match;
        }
    }
    ...
    if (FunctionEntry) {
        ...
        RtlVirtualUnwind(UNW_FLAG_EHANDLER,
                         ...
                         &EstablisherFrame,
                         &ContextPointers);
        LogMessage("RaiseException: throw site rip=%#llx rva=%#llx base=%#llx",
                   (unsigned long long)MSContext.Rip,
                   (unsigned long long)(MSContext.Rip - ImageBase),
        ...
        if (!RtlDispatchException(&ExceptionRecord, &MSContext)) {
            if (cxx_exception && swallow_cxx && swallow_type) {
                LogMessage("RaiseException: swallowing unhandled C++ exception type %s",
